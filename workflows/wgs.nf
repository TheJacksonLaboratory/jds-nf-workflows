...
if(!params.bam_input)  {
  ...
  // Align using read channel to create bam channel
  ...
  bam_file = bam_input_ch.map{it -> [it[0], it[1]]}
  index_file = bam_input_ch.map{it -> [it[0], it[2]]}
...
// If coverage cap is used, run it on the dedup bam file
