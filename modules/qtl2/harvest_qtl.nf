process HARVEST_QTL {
    ...
    publishDir "${params.pubdir}/${id}", pattern:"peaks.csv", mode:'copy'
    publishDir "${params.pubdir}/${id}", pattern:"*_scan1_thresh.png", mode:'copy'

    input:
    tuple val(id), path(perm_files), path(scan1_files), path(cross_files)
