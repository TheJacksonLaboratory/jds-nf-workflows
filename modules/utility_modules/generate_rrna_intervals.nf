process GENERATE_RRNA_INTERVALS {
    ...
    input:
        ...
        path(dict)
    output:
        path("${gtf.baseName}.rRNA_intervals.list"), emit: rRNA_intervals

    script:
        """
        grep "@" ${dict} > ${gtf.baseName}.rRNA_intervals.list
        
        grep 'gene_biotype' ${gtf} | \
            grep "rRNA" | \
            sed -e 's/;/\t/g' | \
            cut -f 1,4,5,7,9 | \
            sed 's/ID=//g' >> ${gtf.baseName}.rRNA_intervals.list
        """
}
