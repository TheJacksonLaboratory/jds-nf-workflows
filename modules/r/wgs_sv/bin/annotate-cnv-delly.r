...
readCNV = function(f, chr=NULL) {
  ...
  colnames(x) <- c('chrom', 'start', 'end', 'ID', 'RDCN', 'RDSD', 'CN')
  # [1]CHROM	[2]POS	[3]END	[4]ID	[5]fizzbang--t_bang:RDCN	[6]fizzbang--t_bang:RDSD	[7]fizzbang--t_bang:CN

  x$log2 <- round(log2(x$RDCN / 2), 4)
...
readCytoband = function(f) {
  ...
  colnames(x) = c('chrom', 'start', 'end', 'cytoband')
...
## Build info field 
cnv$info = paste0('known=',cnv$db, ';DisruptL=',cnv$disrupt.l, ';DisruptR=', cnv$disrupt.r)
